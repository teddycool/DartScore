...
class Board(object):
    ...
    def findSectorLines(self, img):
        ...
        lines = cv2.HoughLines(dst,1,np.pi/180,130)
                        ...
                        try:
                            ...
                            if cross[0] < self.imageX/2+offsetx and cross[0] > self.imageX/2 -offsety:
                                if cross[1] < self.imageY/2+offsety and cross[1] > self.imageY/2 - offsety :
                                    ...
                                    cv2.line(img,self._ilines[i][0],self._ilines[i][1],self._scalibrate,1)
...
if __name__ == "__main__":
    ...
    cam = cv2.VideoCapture(r'C:\Users\par\OneDrive\Documents\GitHub\DartScore\Testdata\Videos\dartscore_20191107_152701.avi')
    (ret, frame) = cam.read()
    index = 0
    ...
    heigth = cam.get(cv2.CV_CAP_PROP_FRAME_HEIGHT)
